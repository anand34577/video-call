...
```powershell
irm https://raw.githubusercontent.com/anand34577/vision-call/main/docker-compose.yml -OutFile docker-compose.yml
```

To change a setting, create a file called `.env` next to `docker-compose.yml` with the lines you need (see [Configuration](Configuration) or the full list in [.env.example](https://github.com/anand34577/vision-call/blob/main/.env.example)), then run `docker compose up -d` again.

To pin a version instead of following `latest`, add `VISIONCALL_VERSION=v1.0.1` to `.env`.
...
Everything (database, uploaded files, the certificate and the session key) is stored in the `visioncall-data` volume. Removing and recreating the container keeps it. To remove the data as well, run `docker volume rm visioncall-data`.

Prefer a folder you can see? Use `-v ./visioncall-data:/data` instead. The container runs as user ID 10001, so give it the folder first:

```bash
mkdir visioncall-data && sudo chown 10001:10001 visioncall-data
```
...
```bash
docker pull ghcr.io/anand34577/vision-call:latest
docker rm -f visioncall
...
```bash
git clone https://github.com/anand34577/vision-call.git
cd vision-call
docker build -t visioncall .
```

Then use `visioncall` instead of `ghcr.io/anand34577/vision-call:latest` in the commands above.
